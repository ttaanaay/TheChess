...
class PieceTemplates private constructor(
    ...
    companion object {
        ...
        /** Loads templates auto-generated by TemplateGenerator, if a complete set exists. */
        fun loadGenerated(context: Context): PieceTemplates? {
            val dir = generatedDir(context)
            val map = mutableMapOf<String, Bitmap>()
            for (label in LABELS) {
                val file = java.io.File(dir, "$label.png")
                if (!file.exists()) return null // incomplete set -- caller should fall back
        ...
        /** Persists a freshly generated template set (see TemplateGenerator) to internal storage. */
        fun saveGenerated(context: Context, bitmaps: Map<String, Bitmap>) {
            val dir = generatedDir(context).apply { mkdirs() }
            for ((label, bmp) in bitmaps) {
                java.io.FileOutputStream(java.io.File(dir, "$label.png")).use {
                    bmp.compress(Bitmap.CompressFormat.PNG, 100, it)
                }
            }
        }

        fun hasGenerated(context: Context): Boolean = LABELS.all { java.io.File(generatedDir(context), "$it.png").exists() }
    }
}
